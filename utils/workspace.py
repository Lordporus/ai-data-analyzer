...
from utils.auth import _get_supabase_client

# Local Fallback Destination
LOCAL_DB_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "outputs", "local_db")
...
def create_organization(name: str) -> Dict:
    """
    Creates a new team organization. Uses Supabase if active,
    otherwise falls back to local database.
    """
...
def get_organizations() -> List[Dict]:
    """
    Fetches all available team organizations.
    """
    client = _get_supabase_client()
    if client:
        try:
    ...
    if not orgs:
        # Default org if none exists
        default_org = create_organization("Default Team Workspace")
        return [default_org]
    return orgs

def add_analysis_run(org_id: str, user_id: str, dataset_name: str, status: str, output_path: str) -> Dict:
    """
    Records an analysis job run metadata.
    """
...
def get_org_analysis_history(org_id: str) -> List[Dict]:
    ...
    if client:
        try:
            res = client.table("analysis_runs").select("*").eq("org_id", org_id).order("created_at", desc=True).execute()
            if res.data:
                return res.data
        except Exception as e:
