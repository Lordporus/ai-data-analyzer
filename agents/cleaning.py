...
class CleaningAgent(BaseAgent):
    ...
    def _execute(self, input_data: IngestionResult, handle_missing: str = "impute") -> CleaningResult:
        ...
        # Guard: if the CSV had no data rows, skip all cleaning steps cleanly.
        if df.empty:
            self._log("DataFrame has no rows — skipping all cleaning steps.")
        ...
        for col in str_cols:
            if col in df.columns:
                ...
                df[col] = (
                    df[col]
                    .where(df[col].isnull(), df[col].astype(str).str.strip().str.replace(r"\s+", " ", regex=True))
                )
